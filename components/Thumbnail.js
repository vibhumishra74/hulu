...
import { forwardRef } from 'react'
 ...
 const Thumbnail = forwardRef(({result},ref)=> {
    // console.log('object thumbnail',result)
    ...
    return (
        ...
        className='group p-2 transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50'>
            ...
            <p className='flex item-center opacity-0 group-hover:opacity-100'>{result.media_type && `${result.media_type} .`}{" "} {`${result.release_date || result.first_air_date}`} .{" "} <ThumbUpIcon className='h-5 mx-2' /> {result.vote_count}</p>
            </div>
        </div>
    )
})
